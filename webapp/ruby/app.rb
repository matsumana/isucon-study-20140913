...
class Isucon3App < Sinatra::Base
  ...
  get '/' do
    ...
    memos = mysql.query("SELECT memos.id, memos.content, users.username, memos.created_at FROM memos  LEFT JOIN users ON users.id = memos.user WHERE is_private=0 ORDER BY created_at DESC, id DESC LIMIT 100")
  ...
  get '/recent/:page' do
    ...
    memos = mysql.xquery("SELECT memos.id, memos.content, users.username, memos.created_at FROM memos LEFT JOIN users ON users.id = memos.user WHERE is_private=0 ORDER BY created_at DESC, id DESC LIMIT 100 OFFSET #{page * 100}")
    if memos.count == 0
